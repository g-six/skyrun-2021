...
export function PlacesInput(props: Props) {
    ...
    useEffect(() => {
        if (props.setAttributes && props.attributes) {
            ...
            if (street_number && street_name && (props.attributes.street_1 != [street_number, street_name].join(' ') || !props.attributes.street_1)) {
            ...
            if (city_state_1 && (!city_state_2 || (city_state_2 && city_state_1.length < city_state_2.length))) {
    ...
    }, [value, address, city, city_state_1, city_state_2, street_number, street_name, country, zip])
    ...
    return <div className="relative">
        <input type="text" className={
            ...
            onFocus={() => { showTextField(false); input_ref.current?.focus() }}
            value={(street_number && street_name && [street_number, street_name].join(' ')) || props.attributes?.street_1 as string || ''}
            readOnly
        />
        <AutoPlaces
            className={
            ...
            onPlaceSelected={(place) => {
                ...
                if (place.address_components) {
                    ...
                    place.address_components.forEach(({ long_name, types }: { long_name: string, types: string[] }) => {
