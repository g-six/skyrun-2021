...
function EmailEditor() {
    ...
    const [tagsDropDownTopPosition, setTagsDropDownTopPosition] =
        useState(0)
    const [tagsDropDownLeftPosition, setTagsDropDownLeftPosition] =
        useState(0)

    const txtEmailSubject =
    ...
    const handleKeyUpEvent = (e: KeyboardEvent<HTMLInputElement>) => {
        if (e.code === 'Slash') {
            setTagsDropDownTopPosition(
                txtEmailSubject.current.getBoundingClientRect().top -
                    30 +
                    document.documentElement.scrollTop
            )
            setTagsDropDownLeftPosition(
                txtEmailSubject.current.getBoundingClientRect().left +
                    30 +
        ...
        <Dashboard>
            <div
                className={classNames(
                    'py-2 px-8 w-full text-black overflow-hidden'
                    //showEmailSettings ? 'overflow-hidden' : '',
                )}
            >
                ...
                <div className="flex gap-3">
                    {languages.map((lang) => (
                        <>
                            ...
                            >
                                <i
                                    className={classNames(
                                        `flag-icon flag-icon-${lang.flag} flag-icon-rounded text-3xl rounded-full h-4 w-4`
                                    )}
                                />
                                <Translation
                                    render_as="div"
                    ...
                    >
                        <option
                            ...
                            disabled
                            selected
                            className="add-language-placeholder"
                        >
                            +&nbsp;&nbsp; ADD LANGUAGE
                        </option>
                ...
                <div className="w-9/12">
                    ...
                    />
                    <span ref={widthRef} className="invisible">
                        &nbsp;{emailSubject}&nbsp;
                    </span>
                    <Translation
                        render_as="div"
            ...
            <EmailSettings
                handleShow={setShowEmailSettings}
                isShown={showEmailSettings}
            />
        </Dashboard>
    )
}
